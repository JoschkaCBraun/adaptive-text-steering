...
def main():
    ...
    for act_idx, act_type in enumerate(activation_types):
        ...
        steering_vectors = sv.compute_contrastive_steering_vector(
        ...
        for layer_idx, layer in enumerate(layers):
            ...
            pos_proj = project_onto_steering_vector(
                activations['positive'][layer], 
                steering_vectors[layer]
            )
            neg_proj = project_onto_steering_vector(
    ...
    output_file = os.path.join(
        plots_path, 
        f'positive_negative_projection_distribution_{model}_{dataset}_{num_samples}.pdf'
    )
