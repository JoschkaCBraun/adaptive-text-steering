...
datasets = d.all_datasets_with_index_and_fraction_figure_13_tan_et_al
...
ax1.scatter(x_vals_score * 100, y_vals, color='blue', marker='o',
           label=f'Pearson correlation: {correlation_score:.3f}\np-value: {p_value_score:.3e}')
ax1.set_xlabel('Fraction Anti-Steerable (%)')
ax1.set_ylabel('Layer 13 Average Cosine Similarity')
ax1.set_title('Fraction Anti-Steerable vs Layer 13 Cosine Similarity')
...
ax2.scatter(x_vals_index, y_vals, color='red', marker='s',
           label=f'Pearson correlation: {correlation_index:.3f}\np-value: {p_value_index:.3e}')
ax2.set_xlabel('Dataset Index')
ax2.set_ylabel('Layer 13 Average Cosine Similarity')
ax2.set_title('Dataset Index (Steerability) vs Layer 13 Cosine Similarity')
ax2.legend(loc='lower left')
ax2.grid(True)

plt.tight_layout()
os.makedirs(plots_path, exist_ok=True)
plt.savefig(os.path.join(plots_path, f'steerability_and_index_correlation_figure_13_{NUM_SAMPLES}.pdf'))
plt.close()