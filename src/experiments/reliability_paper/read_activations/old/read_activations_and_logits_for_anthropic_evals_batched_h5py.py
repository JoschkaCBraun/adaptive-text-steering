...
def collect_last_activations_and_logits_for_batch(
    ...
    with torch.inference_mode(), steering_vectors.record_activations(
        model,
            ...
            for start_idx in range(0, num_samples, batch_size):
                ...
                if not is_prefilled:
                    ...
                    batch_prompts_matched = [
                        p + a for p, a in zip(batch_prompts, batch_matching_answers)
                    ]
                    ...
                    batch_prompts_non_matched = [
                        p + a for p, a in zip(batch_prompts, batch_non_matching_answers)
                    ]
                ...
                save_batch_results(
                    ...
                    batch_results,
                    batch_prompts,
                    batch_matching_answers,
                    batch_non_matching_answers,
                    is_prefilled
                )

                # Update sample count
                h5_file.attrs['num_samples'] += len(batch_prompts)
                
                # Free memory
                del batch_results
                torch.cuda.empty_cache()
                
                # logging.info(f"  Saved batch {start_idx}-{end_idx} for scenario '{scenario_name}' in {output_file}")
    end_time = time()
