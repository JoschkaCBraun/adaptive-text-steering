...
def create_violin_plot(data: List[List[float]], scenarios: List[str], 
    ...
    ax.set_title('Per-sample steering effect size by prompt types', fontsize=12)
    ...
    # make plot as tight as possible
    plt.tight_layout()
...
def process_and_plot_results(input_dir: str, output_dir: str, num_datasets: int) -> None:
    ...
    output_path = os.path.join(output_dir, 'steering_vectors_from_all_prompts_work_equally_well_logits.pdf')
    create_violin_plot(avg_data, plot_scenarios, first_dataset['eval_samples'], output_path)
