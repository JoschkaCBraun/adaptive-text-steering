...
def create_violin_subplot(
    ...
    ax.set_ylabel('Logit difference\nrelative to no steering', fontsize=7.5)
    ...
    ax.legend(
        ...
        loc='center left',
        bbox_to_anchor=(1.01, legend_position),
        fontsize=8,
        title_fontsize=8
    )
...
def main():
    ...
    # We'll just define an "average" group across many datasets
    average_datasets = all_datasets_figure_13
    ...
    fig, axes = plt.subplots(2, 1, figsize=(6, 3.3))
    fig.suptitle("Per-sample steering effect size by prompt type", fontsize=12)
    
    # 1) Subplot for Best (group 1-6): hide xlabels
    create_violin_subplot(
        ...
        show_xlabels=False,
        legend_position=0.61
    )
    
    # 2) Subplot for Average (all considered)
    create_violin_subplot(
        ax=axes[1],
        data=avg_data,
        scenarios=plot_scenarios,
    ...
    out_file = os.path.join(output_dir, "two_subplots_vertical.pdf")
    plt.savefig(out_file, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"Saved 2-subplot figure to: {out_file}")
