# -*- coding: utf-8 -*-
'''
This module provides utilities for loading sentiment vector training data (sentences or words).
It reads predefined CSV files containing pairs of positive and negative examples.
'''
...
MAX_SENTENCES = 500
MAX_WORDS = 2800 # Note: Original file might contain ~2370 based on previous check, using 2800 from filename convention. Adjust if needed.

DEFAULT_LANGUAGE = 'en'
# --- End Constants ---

def get_sentiment_representation_file_name(representation_type: str, language: str = DEFAULT_LANGUAGE) -> str:
    '''
    ...
    if representation_type == SENTIMENT_SENTENCES:
        ...
        return f"{representation_type}_{MAX_SENTENCES}_{language}.csv"
    elif representation_type == SENTIMENT_WORDS:
        # Assumes filename format includes the max number of samples
        return f"{representation_type}_{MAX_WORDS}_{language}.csv"
    else:
        raise ValueError(f"Invalid representation type: '{representation_type}'. "
                         f"Expected one of {VALID_REPRESENTATION_TYPES}")

def load_sentiment_vector_training_samples(
    ...
    if representation_type == SENTIMENT_SENTENCES:
        ...
        if language != DEFAULT_LANGUAGE:
            raise ValueError(f"Language '{language}' is not supported for {SENTIMENT_WORDS}. "
                             f"Only '{DEFAULT_LANGUAGE}' is available.")
    ...
    try:
        ...
        sub_dir = representation_type # Assumes subdirectories match representation type names
...
def main() -> None:
    ...
    Requires the 'SENTIMENT_DATA_PATH' environment variable to be set.
    Example:
    export SENTIMENT_DATA_PATH='/path/to/your/sentiment/data'
    ...
    try:
        ...
        samples_sent = load_sentiment_vector_training_samples(
        ...
        samples_sent_small = load_sentiment_vector_training_samples(
            representation_type=SENTIMENT_SENTENCES,
            num_samples=10
        )
        ...
        print("-" * 30)
        print(f"Loading {SENTIMENT_WORDS} (up to {MAX_WORDS}, language='{DEFAULT_LANGUAGE}')...")
        samples_word = load_sentiment_vector_training_samples(
