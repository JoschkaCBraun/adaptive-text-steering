...
def generate_text_with_steering_vector(model: AutoModelForCausalLM, tokenizer: AutoTokenizer, prompt: str, steering_vector: torch.Tensor, steering_strength: float, device: torch.device) -> Dict[str, Union[int, str]]:
    """
    Generates text with a steering vector applied to the model during generation.
    """
